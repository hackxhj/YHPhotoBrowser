...
@property (nonatomic,strong) YHWebImageLoadView *loadView;
@end
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    _mainUrl=url;
    [self resetImgView];
    [self.loadView loading];
    [self.showImg addSubview:self.loadView];
...
-(YHWebImageLoadView *)loadView
{
    if(!_loadView){
        _loadView=[[YHWebImageLoadView alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2-25, [UIScreen mainScreen].bounds.size.height/2-25, 50, 50)];
        [self.showImg addSubview:_loadView];
    }
    return _loadView;
}
@end
